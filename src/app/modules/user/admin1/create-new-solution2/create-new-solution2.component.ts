...
export class CreateNewSolution2Component implements OnInit {
  ...
  // createExpressionList = [{ columnName: '', expression: '', isEdit: false, selectedDataType: '', where: '', or: '', groupBy: '', value: '', signOperator: '', whereList: [{ operator: '', signOperator: '', value: '' }] }];
  //fileKeys: any[] = [];
  //activeTab: any;
  //selectedInputFileList: any[] = [];
  //selectedOtherFileName: any;
  //showTableData: any[] = [];
  //checkedSheet: any;
  //checkedFilesList: any;
  //showTab02:boolean = false;
  filesList: any[] = [];
  //fileNameList: any[] = []; //x
  columnsArr: any[] = [];
  ...
  //showTargetFilesColumns: any[] = []; //tab4
  ...
  add_SaveCreateExpression(obj: any) {
    ...
    if (obj.sourceFieldName.includes('-')) {
      ...
      const filteredFile = this.fileListtt.filter(
    ...
    if (obj.expression == '' || obj.expression?.length == 0)
      ...
      if (obj.expression.includes('-')) {
        ...
        const fName = fSName[0];
        const sName = fSName[1];
        const filteredFile = this.fileListt.filter(
          (x: any) => x.fileName == fName
        );
  ...
  applyFilter(event: any) {
    this.messagesSearch = (this.messages.filter((user: any) => this.containsValue(user, event.target.value.trim().toLowerCase())));
    // // this.usersSearch = new MatTableDataSource<any>(res.responseData);
    // this.SubSearch.paginator = this.paginator;
  ...
  // selectPrimaryTable(e: any, item: any, i: number) {
  //   const value = e.value.value;
  //   this.mergeCheckedList[i].dropDownList.primaryTableList.forEach((priTab: any) => priTab.isSelected = false)
  //   e.value.isSelected = true;
  //   const selectedPT = this.mergeCheckedList[i].dropDownList.primaryTableList.filter((x: any) => x.value == value)
  //   this.mergeCheckedList[i].dropDownList.primaryKeyList = selectedPT[0].sheet;
  // }

  // selectForeignTable(e: any, item: any, i: number) {
  //   const value = e.value.value;
  //   this.mergeCheckedList[i].dropDownList.primaryTableList.forEach((priTab: any) => priTab.isSelected = false)
  //   e.value.isSelected = true;
  //   const selectedPT = this.mergeCheckedList[i].dropDownList.primaryTableList.filter((x: any) => x.value == value)
  ...
  // selectPrimarykey(e: any, item: any, j: number) {
  //   const value = e.value;
  // }
  // selectForeignKey(e: any, item: any, j: number) {
  //   const value = e.value;
  // }

  // previewPopup(j: number, item: any, i: number, obj: any) { //filtered files, 
  ...
  //   let jf: any[] = []; let af: any[] = []; let sf: any[] = [];
  //   if (selectedList.filter((x: any) => x.fileType == 'join file').length != 0) {
  //     jf = selectedList.filter((x: any) => x.fileType == 'join file').map((y: any) => (y.joinName + '.' + y.columnName))
  //   }
  //   if (selectedList.filter((x: any) => x.fileType == 'append file').length != 0) {
  //     af = selectedList.filter((x: any) => x.fileType == 'append file').map((y: any) => (y.appendName + '.' + y.columnName))
  //   }
  //   if (selectedList.filter((x: any) => x.fileType == 'source file').length != 0) {
  //     sf = selectedList.filter((x: any) => x.fileType == 'source file').map((y: any) => (y.tableName + '.' + y.columnName))
  //   }
  //   let allFiles = [...jf, ...af, ...sf]
  ...
  groupbyOrderby() {
    const expandedDataTable = this.datasetDetailsList.filter((x: any) => x.isDatasetTableExpand == true)
    this.selectedColumnsOfDS = [];
    expandedDataTable[0]?.datasetFieldDTOList.forEach((ds: any) => {
      const cNameInDS = ds.columnName.trim();
      if (cNameInDS.includes('[')) {
        const splitCName = cNameInDS.split(/[[]+/);
        const fileNameinDS = splitCName[0];
        const sheetNameinDS = splitCName[1].slice(0, -1);
        ...
        this.AJFDlist?.forEach((ajfd: any) => {
          if (fileNameinDS == ajfd.fileName) {
            if (this.selectedColumnsOfDS.filter((x: any) => x.fileName == fileNameinDS).length == 0) {
              this.selectedColumnsOfDS.push(ajfd)
            }
          }
        })

        // this.AJFDlist.filter((element: any) =>
