...
export class AddServiceComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.getAllProcess()
    this.getByInternalUserIdSubscritpionDetails()
  }
  getAllProcess() {
  ...
  getByInternalUserIdSubscritpionDetails() {
    // let data = this.id;
    // let data = JSON.parse(localStorage.getItem("userInfo") || '{}')
    // console.log(data, data.userId);
    this.Master.getByInternalUserIdSubscritpionDetails(this.userData.userId).subscribe((res: any) => {
  ...
  addserviceSubmit() {
    this.submited = true
    if (this.processId.length == 0 || this.subscriptionTypeId.length == 0) {
